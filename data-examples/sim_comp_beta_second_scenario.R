...
### Data generated from a spatial generalized linear mixed models
...
for (j in seq_along(range_grid)) {
  ...
  true_mu <- plogis(cbind(1, XX) %*% true_bb + gp)
  yy <- rbeta(nn, true_mu * true_psi, (1 - true_mu) * true_psi)
...
for (j in seq_along(range_grid)) {
  ...
  coords <- ref_dat[,1:2]
  max_edge <- diff(range(coords)) / (3 * 5) / 2
...
write_file(tbl_latex, paste0(outpath, "beta_s2_pred_comp.txt"))